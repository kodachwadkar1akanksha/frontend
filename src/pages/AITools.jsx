...
const AITools = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">AI Evidence Analysis Tools</h1>
        <p className="text-slate-400">Advanced AI-powered analysis for CCTV footage and forensic evidence</p>
      </div>
          ...
          <CardContent className="space-y-4">
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-8 text-center">
              ...
              <div className="space-y-2">
                <p className="text-lg font-medium text-white">Upload your evidence file</p>
            ...
            {isProcessing && (
              <div className="space-y-2">
                ...
                <div className="w-full bg-slate-700 rounded-full h-2">
                  <div className="bg-police-blue h-2 rounded-full animate-pulse" style={{ width: '75%' }}></div>
                </div>
              </div>
            )}
          </CardContent>
              ...
              <TableBody>
                {results.map((result) => (
                  <TableRow key={result.id}>
                    ...
                    <TableCell className="text-slate-300">{result.details}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
            ...
            <div className="relative">
              <div className="flex items-center space-x-4 overflow-x-auto pb-4">
                {results.map((result, index) => (
                  <div key={result.id} className="flex flex-col items-center space-y-2 min-w-0">
                    <div className={`w-4 h-4 rounded-full ${
                      result.alert === 'high' ? 'bg-red-400' : 
                      result.alert === 'medium' ? 'bg-yellow-400' : 'bg-green-400'
                    }`}></div>
